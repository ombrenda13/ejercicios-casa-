...
INSTRUCCIONES:
	flecha izquierda--rotar todo hacia izquierda
	flecha derecha-- rotar  todo hacia  derecha
	flecha arriba-- rotar  todo hacia  hacia arriba
	flecha abajo--- rotar  todo hacia  hacia abajo
...
void display(void)
{
	...
	projectionShader.setMat4("model", model);
	projectionShader.setVec3("aColor", glm::vec3(1.7f , 0.9f , 0.6f));
	sphere_draw();
	model = glm::mat4(1.0f);//inicilizar la matriz para que la posicion anterior no afecte hacer esto para todos los planetas

	//tierra---------------------------	
	model = glm::rotate(model, glm::radians(tierra), glm::vec3(1.0f, 0.0f, 0.0f));//traslacion
	...
	projectionShader.setMat4("model", model);
	projectionShader.setVec3("aColor", glm::vec3(0.0f, 0.2f, 0.67f));
	sphere_draw();
	model = glm::mat4(1.0f);//inicilizar la matri
	
	//venus---------------------------	
	model = glm::rotate(model, glm::radians(venus), glm::vec3(1.0f, 1.0f, 1.0f));//traslacion
	model = glm::translate(model, glm::vec3(3, 0, 0));
	...
	projectionShader.setMat4("model", model);
	projectionShader.setVec3("aColor", glm::vec3(1.0f, 0.9f, 0.2f));
	sphere_draw();
	model = glm::mat4(1.0f);//inicilizar la matri

	//marte---------------------------	
	model = glm::rotate(model, glm::radians(marte), glm::vec3(-1.0f, -1.0f, -1.0f));//traslacion
	...
	model = glm::mat4(1.0f);//inicilizar la matri
	//saturno
	model = glm::rotate(model, glm::radians(saturno), glm::vec3(-1.0f, 0.0f, 0.0f));//traslacion
	model = glm::translate(model, glm::vec3(0, 4, 0));
	model = glm::rotate(model, glm::radians(saturno), glm::vec3(0.0f, 1.0f, 0.0f));//rotación
	temp01 = model;
	model = glm::scale(model, glm::vec3(0.7f, 0.7f, 0.7f));
	projectionShader.setMat4("model", model);
	projectionShader.setVec3("aColor", glm::vec3(0.2f, 0.7f, 0.4f));
	sphere_draw();
	model = temp01;
	//anillo	
	...
	//urano
	model = glm::rotate(model, glm::radians(urano), glm::vec3(-1.0f, -1.0f, -1.0f));//traslacion
	model = glm::translate(model, glm::vec3(3, 0, 0));
	...
	projectionShader.setMat4("model", model);
	projectionShader.setVec3("aColor", glm::vec3(0.6f, 0.6f, 0.9f));
	sphere_draw();
	model = temp01;
